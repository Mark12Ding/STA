...
class Block(nn.Module):
    ...
    def forward(self, x):
        if self.gamma_1 is None:
            new, metric = self.attn(self.norm1(x))  
            x = x + self.drop_path(new)
...
class VisionTransformer(nn.Module):
    ...
    def merge(self, x, metric, index, depth=12):
        B, N, D_x = x.shape
        r = self.r 
        if index == 0:
            T = 8
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            x, metric = blk(x)
            if self.r > 0:
                x = self.merge(x, metric, i, len(self.blocks))
